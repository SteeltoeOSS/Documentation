...
If not, have a look at the resources listed in [further-reading](./further-reading.html).
...
For much more information about these and the other Exchange types, see [further-reading](./further-reading.html).
...
You can learn more about the default Exchange's usage within Steeltoe RabbitMQ in [RabbitTemplate](./rabbitmq-reference.html#rabbittemplate).
...
However, as you will see later in [Configuring the Broker](./rabbitmq-reference.html#configuring-the-broker), the `RabbitAdmin` class can use `IBinding` instances to actually trigger the binding actions on the broker.
...
The `RabbitTemplate` mentioned earlier is one of the main components involved in actual RabbitMQ messaging, and it is discussed in detail in its own section[Rabbit Template](./rabbitmq-reference.html#rabbittemplate).
...
>When the cache mode is `CONNECTION`, automatic declaration of queues and other AMQP types (See [Automatic Declaration of Exchanges, Queues, and Bindings](./rabbitmq-reference.html#automatic-declaration-of-exchanges-queues-and-bindings) is NOT supported.
...
See [Using a Separate Connection](./rabbitmq-reference.html#using-a-separate-connection) for more information.
...
See also `PublisherConfirms` in [Scoped Operations](./rabbitmq-reference.html#scoped-operations).
...
See [Publishing is Asynchronous — How to Detect Successes and Failures](./rabbitmq-reference.html#publishing-is-asynchronous-how-to-detect-success-and-failure) for one scenario where you might want to register a `IChannelListener`.
...
Also, as discussed in [Configuring the Broker](./rabbitmq-reference.html#configuring-the-broker), `RabbitAdmin` re-declares any infrastructure services (queues and others) when the connection is re-established.
...
If you have a need for a variable number of declarations, consider defining a service, of type `IDeclarable`, as discussed in [Declaring Collections of Exchanges, Queues, and Bindings](./rabbitmq-reference.html#declaring-collections-of-exchanges-queues-and-bindings).
...
We will explore message sending and reception, respectively, in [Sending Messages](./rabbitmq-reference.html#sending-messages) and [Receiving Messages](./rabbitmq-reference.html#receiving-messages).
...
For successful publishing, you can receive an asynchronous confirm, as described in [Template Publisher Confirms and Returns](./rabbitmq-reference.html#template-publisher-confirms-and-returns).
...
The first case is covered by publisher returns, as described in [Template Publisher Confirms and Returns](./rabbitmq-reference.html#template-publisher-confirms-and-returns).
...
For returned messages, the template's `Mandatory` property must be set to `true`. See [Factory Publisher Confirms and Returns](./rabbitmq-reference.html#factory-publisher-confirms-and-returns)
...
Only one `ReturnCallback` is supported by each `RabbitTemplate`. See also [Reply Timeout](./rabbitmq-reference.html#reply-timeout)
...
See also [Scoped Operations](./rabbitmq-reference.html#scoped-operations) for a simpler mechanism for waiting for publisher confirms.
...
If the channel is a `IPublisherCallbackChannel`, it is returned to the cache after all confirms have been received (see [Template Publisher Confirms and Returns](./rabbitmq-reference.html#template-publisher-confirms-and-returns).
...
Note that a higher-level abstraction that lets you correlate confirms to requests is provided elsewhere (see [Template Publisher Confirms and Returns](./rabbitmq-reference.html#template-publisher-confirms-and-returns).
...
When publisher confirms are enabled, this object is returned in the callback described in [RabbitTemplate](./rabbitmq-reference.html#rabbittemplate).
...
When the template's `Mandatory` property is set to `true`, returned messages are provided by the callback described in [RabbitTemplate](./rabbitmq-reference.html#rabbittemplate).
...
See [Reply Timeout](./rabbitmq-reference.html#reply-timeout) for more information.
...
However, see [RabbitListener with Batching](./rabbitmq-reference.html#rabbitListener-with-batching) for more information.
...
See [Message Converters](./rabbitmq-reference.html#message-converters) for more information.
...
See [Attribute driven Listener Endpoints](./rabbitmq-reference.html#attribute-driven-listener-endpoints) for more details.
...
See [Message Listener Container Configuration](.rabbitmq-reference.html#message-listener-container-configuration).
...
See [Attribute Driven Listener Endpoints](./rabbitmq-reference.html#attribute-driven-listener-endpoints) for more information.
...
See [SimpleMessageConverter](./rabbitmq-reference.html#simplemessageconverter) for more information and information about other converters available.
...
You can modify the queues on which the container listens at runtime. See [Listener Container Queues](./rabbitmq-reference.html#listener-container-queues).
...
You can also use conditional declaration (see [Conditional Declaration](./rabbitmq-reference.html#conditional-declaration) together with setting `AutoStartup = "false"` on the `RabbitAdmin` to defer queue declaration until the container is started.
The following example shows how to do so:
...
Rejecting any message from a batch causes the entire batch to be rejected.
See [Batching](./rabbitmq-reference.html#batching) for more information about batching.
...
See also [Logging Channel Close Events](./rabbitmq-reference.html#channel-close-logging).
...
See [Message Listener Container Configuration](./rabbitmq-reference.html#message-listener-container-configuration) for details on how to set this up.
...
See [Listening to Multiple Queues](./rabbitmq-reference.html#listening-to-multiple-queues) for examples.
...
See [Reply Management](./rabbitmq-reference.html#reply-management) for information about replies.
...
See [JsonMessageConverter](./rabbitmq-reference.html#jsonmessageconverter) for more information.
...
>IMPORTANT: For multi-method listeners ( see [Multiple Method Listeners](./rabbitmq-reference.html#multiple-method-listeners) ), the method selection is based on the payload of the message *after the message conversion*.
...
Notice that the same method signatures apply, as discussed in the method-level `[RabbitListener()]` [described earlier](./rabbitmq-reference.html#messagelisteneradapter).
...
When receiving a [batch](./rabbitmq-reference.html#batching) of messages, the de-batching is normally performed by the container and the listener is invoked with one message at at time.
...
A batch-enabled factory cannot be used with a [multi-method listener](./rabbitmq-reference.html#multiple-method-listener).
...
Listener container factories are used to support the `[RabbitListener()]` attribute and registering containers with the `IRabbitListenerEndpointRegistry`, as discussed [above](./rabbitmq-reference.html#programmatic-endpoint-registration).
...
>The `MessageListenerAdapter` mentioned in [Asynchronous Consumer](./rabbitmq-reference.html#asynchronous-consumer) also uses a `IMessageConverter`.
...
* [Converting to IMessage JsonMessageConverter](./rabbitmq-reference.html#converting-to-imessage-jsonMessageConverter)
* [Converting from IMessage JsonMessageConverter](./rabbitmq-reference.html#converting-from-imessage-jsonMessageConverter)
...
As can be seen in [Message Converters](./rabbitmq-reference.html#message-converters), one such extension point is in the `RabbitTemplate` `ConvertAndReceive` operations, where you can provide a `IMessagePostProcessor`.
...
When using batching (see [Batching](./rabbitmq-reference.html#batching), this is invoked after the batch is assembled and before the batch is sent.
...
See [Template Publisher Confirms and Returns](./rabbitmq-reference.html#template-publisher-confirms-and-returns) and the [`RabbitTemplate`](https://github.com/SteeltoeOSS/Steeltoe/blob/master/src/Messaging/src/RabbitMQ/Core/RabbitTemplate.cs) code for more detail.
...
You can enable it by setting `PublisherReturns` to `true` on the `CachingConnectionFactory` (see [Factory Publisher Confirms and Returns](./rabbitmq-reference.html#factory-publisher-confirms-and-returns).
...
>When the template does not use a fixed `ReplyAddress` (or is using [Direct Reply-To](./rabbitmq-reference.html#direct-reply-to), a listener container is not needed and a internal temp container is created.
...
See also [Scoped Operations](./rabbitmq-reference.html#scoped-operations).
...
See [Containers and Broker Named Queues](./rabbitmq-reference.html#containers-and-broker-named-queues) for more information.
...
See [AnonymousQueue](./rabbitmq-reference.html#anonymousqueue) to understand why `AnonymousQueue` is preferred over broker-generated queue names as well as how to control the format of the name.
...
See [Automatic Declaration of Exchanges Queues and Bindings](./rabbitmq-reference.html#automatic-declaration-of-exchanges-queues-and-bindings).
...
For queues, if the `*Name` is omitted, an anonymous queue is created [AnonymousQueue](./rabbitmq-reference.html#anonymousqueue).
...
As discussed in [Message Listeners and the Asynchronous Case](./rabbitmq-reference.html#message-listeners-and-the-asynchronous-case), the listener can throw an `RabbitRejectAndDontRequeueException` (or `ImmediateRequeueException`) to conditionally control this behavior.
...
For more information about rejecting failed messages, see [Message Listeners and the Asynchronous Case](./rabbitmq-reference.html#message-listeners-and-the-asynchronous-case).
...
| BatchingStrategy | The strategy used when de-batching messages. Default `SimpleDebatchingStrategy`. See [Batching](./rabbitmq-reference.html#batching) and [RabbitListener with Batching](./rabbitmq-reference.html#rabbitlistener-with-batching). |
...
| ConsumerTagStrategy | Set an implementation of [ConsumerTagStrategy](./rabbitmq-reference.html#consumer-tags), enabling the creation of a (unique) tag for each consumer. |
| ConsumersPerQueue | The number of consumers to create for each configured queue. See [Listener Concurrency](./rabbitmq-reference.html#listener-concurrency). |
