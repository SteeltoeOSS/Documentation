...
Currently, Steeltoe provides a single binder implementation for [Rabbit MQ](https://github.com/spring-attic/spring-cloud-stream-binder-rabbit). Future binders provided by Steeltoe are on the roadmap including Kafka and others.
...
There are two types of application-level error handling. Errors can be handled at each binding subscription or by a global handler which handles all the binding errors. Below are the details.
   ...
   <img src="/docs/v3/stream/images/custom_vs_global_error_channels.png" alt="An Application with Custom and Global Error Handler" width="100%">
