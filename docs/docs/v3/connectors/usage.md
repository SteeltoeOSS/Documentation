...
>** If you [publish your application as a single file](https://learn.microsoft.com/dotnet/core/deploying/single-file/overview) prior to deployment, this assembly will not be included in the output unless there is a reference to it. Package version 3.0.2 adds the no-op method `CloudFoundryConnector.EnsureAssemblyIsLoaded()`, which can be called from anywhere in your application, to support this scenario.
