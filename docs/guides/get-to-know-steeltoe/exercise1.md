...
- The [health actuator](/docs/v3/management/health.md) adds a new endpoint at `/actuator/health`. This endpoint retrieves health contributors that have been registered in the application's service container with either Steeltoe's `IHealthContributor` or Microsoft's `IHealthRegistration` to aggregate the health checks of all the application's dependencies. This endpoint use the HTTP status code of [200](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/200) when everything is OK and [503](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/503) when it isn't. Details of the health checks can be included in JSON-formatted responses, too.
