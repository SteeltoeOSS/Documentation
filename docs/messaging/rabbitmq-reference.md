...
>NOTE: When the template does not use a fixed `ReplyAddress` (or is using [Direct Reply-To](#direct-reply-to), a listener container is not needed and a internal temp container is created.
